...
export default function CancionActivaSection({
  ...
  return (
    <section className="flex min-h-0 flex-1 flex-col overflow-y-auto bg-bg-app py-3 pb-6">
      {hasCancion ? (
        <>
          <h2 className="shrink-0 px-2 text-xl font-extrabold text-text-primary">
            {cancionNombre}
          </h2>
          {artista && (
            <p className="mt-0.5 shrink-0 px-2 text-[13px] text-text-muted">
              {artista}
            </p>
          )}

          {loading && !showExtractedLetra && !showEmbeddedLetra && (
            <div className="mt-4 flex items-center gap-2 px-2 text-sm text-text-muted">
              <Loader2
          ...
          {showExtractedLetra && (
            <div
              className="mt-3 w-full bg-letra-bg px-4 py-5 text-letra-text whitespace-pre-wrap"
              style={{
          ...
          {showEmbeddedLetra && urlLetra && (
            <div className="mt-3 flex w-full flex-col">
              ...
              <a
                ...
                className="mt-3 self-center px-2 text-sm text-text-muted underline-offset-2 hover:underline"
              >
                Abrir en el sitio
              </a>
            </div>
